...
EnterDungeonResult MySqlDungeonRepository::EnterDungeon(const PlayerSnapshot& player_snapshot,
    ...
    result.battle_context.player_id = player_snapshot.player_id;
    ...
    do {
        // Approved ownership exception: dungeon keeps this player-asset update colocated in the
        // single-database enter-dungeon transaction. This must not expand into compile-time
        // dependencies on player module internals.
        std::ostringstream pending_battle_sql;
        pending_battle_sql << "SELECT battle_id FROM dungeon_battle WHERE player_id = " << player_snapshot.player_id
        ...
        update_asset_sql << "UPDATE player_asset SET stamina = stamina - " << dungeon_config.cost_stamina
                         << " WHERE player_id = " << player_snapshot.player_id
        ...
        if (!mysql->Execute(update_asset_sql.str(), &error_message, &affected_rows) || affected_rows != 1) {
            result.error =
                affected_rows == 0 ? DungeonRepositoryError::kStaminaNotEnough : DungeonRepositoryError::kStorageFailure;
    ...
    result.success = true;
    result.remain_stamina = player_snapshot.stamina - dungeon_config.cost_stamina;
    return result;
}
...
SettleDungeonResult MySqlDungeonRepository::SettleDungeon(const common::model::BattleContext& battle_context,
    ...
    bool transaction_ok = false;
    do {
        // Approved ownership exception: reward, stamina-adjacent and dungeon-progress writes remain
        // in this settlement transaction for simplicity and strong consistency. Keep the exception
        // local to dungeon infrastructure rather than leaking player internals across modules.
        std::uint64_t affected_rows = 0;
        ...
        if (!mysql->Execute(update_battle_sql.str(), &error_message, &affected_rows) || affected_rows != 1) {
            result.error =
                affected_rows == 0 ? DungeonRepositoryError::kBattleAlreadySettled : DungeonRepositoryError::kStorageFailure;
