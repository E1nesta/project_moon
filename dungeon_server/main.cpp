...
common::bootstrap::ServiceOptions ParseOptions(int argc, char* argv[]) {
    ...
    options.service_name = "dungeon_server";
    options.config_path = "configs/dungeon_server.conf";
...
int main(int argc, char* argv[]) {
    ...
    logger.SetServiceName("dungeon_server");
    ...
    dungeon_server::DungeonNetworkServer server(config);
    std::string error_message;
    if (!server.Initialize(&error_message)) {
        logger.Log(common::log::LogLevel::kError, "dungeon_server init failed: " + error_message);
        return 1;
    }

    logger.Log(common::log::LogLevel::kInfo,
               "dungeon_server listening on port " + std::to_string(config.GetInt("port", 7300)));
