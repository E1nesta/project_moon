...
public:
    ...
    std::optional<dungeon_server::dungeon::PlayerSnapshot> GetBattleEntrySnapshot(std::int64_t player_id) const override {
        ...
        snapshot.role_summaries = response.role_summaries;
        return snapshot;
    }
    ...
    dungeon_server::dungeon::PrepareBattleEntryPortResponse PrepareBattleEntry(std::int64_t player_id,
    ...
    dungeon_server::dungeon::CancelBattleEntryPortResponse CancelBattleEntry(
    ...
    dungeon_server::dungeon::ApplyRewardGrantPortResponse ApplyRewardGrant(
        std::int64_t player_id,
...
struct DemoOptions {
    ...
    std::string dungeon_config = "configs/demo/dungeon_server.conf";
...
int main(int argc, char* argv[]) {
    ...
    common::mysql::MySqlClientPool battle_mysql_pool(
        ...
        static_cast<std::size_t>(dungeon_config.GetInt("storage.battle.mysql.pool_size", 4)));
    ...
    common::redis::RedisClientPool battle_redis_pool(
        common::redis::ReadConnectionOptions(dungeon_config, "storage.battle.redis."),
        static_cast<std::size_t>(dungeon_config.GetInt("storage.battle.redis.pool_size", 4)));
    ...
    dungeon_server::dungeon::MySqlDungeonRepository dungeon_repository(battle_mysql_pool);
    auto battle_context_repository =
        dungeon_server::dungeon::RedisBattleContextRepository::FromConfig(battle_redis_pool, dungeon_config);
    dungeon_server::dungeon::RedisPlayerLockRepository player_lock_repository(
        battle_redis_pool, dungeon_config.GetInt("storage.player.lock_ttl_seconds", 10));
    ...
    const auto enter_response = dungeon_service.EnterBattle(
    ...
    const auto settle_response = dungeon_service.SettleBattle(
    ...
    const auto grant_status = dungeon_service.GetRewardGrantStatus(settle_response.reward_grant_id);
    ...
    if (options.run_negative_cases) {
        ...
        const auto duplicate_settle = dungeon_service.SettleBattle(
            {login_response.default_player_id, enter_response.session_id, demo_data.stage_id, 3, 1, 123456},
        ...
        const auto invalid_star_enter = dungeon_service.EnterBattle(
        ...
        const auto invalid_star_settle = dungeon_service.SettleBattle(
        ...
        const auto cleanup_settle = dungeon_service.SettleBattle(
