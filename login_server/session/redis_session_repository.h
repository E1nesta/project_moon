...
// Redis-backed implementation of the session storage boundary.
class RedisSessionRepository final : public SessionRepository {
public:
    static RedisSessionRepository FromConfig(common::redis::RedisClientPool& redis_pool,
                                             const common::config::SimpleConfig& config);

    RedisSessionRepository(common::redis::RedisClientPool& redis_pool, int session_ttl_seconds);
